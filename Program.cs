...
foreach (var line in input.Split(Environment.NewLine))
{
    ...
    foreach (var handful in handfuls)
    {
        ...
        {
            if (!possible) { break; }
            //Console.WriteLine($"Game {gameNumber} : {ballsByColor.count} - {ballsByColor.color} - max {bag[ballsByColor.color]}");
            if (bag[ballsByColor.color] < ballsByColor.count)
            {
                Console.WriteLine($"BREAKING on Game {gameNumber} : {ballsByColor.count} - {ballsByColor.color} - max {bag[ballsByColor.color]}");
