...
Dictionary<char, int> ParsePart(string str)
{
    return str.Split('{', '}', ',').Where(x => x != "").Select(x => x.Split("=")).ToDictionary(x => x[0][0], x => int.Parse(x[1]));
}

record Operation(char param, bool greaterThan, int value, string next);
